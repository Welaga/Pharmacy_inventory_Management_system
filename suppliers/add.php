...
requireAdmin();
define('PAGE_TITLE', 'Add Supplier');
$errors = [];
$data   = ['status' => 'active'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        db()->prepare("INSERT INTO suppliers (name,contact,email,address,status) VALUES (?,?,?,?,?)")
            ->execute([$data['name'],$data['contact'],$data['email'],$data['address'],$data['status']]);
        $newId = db()->lastInsertId();
        auditLog('CREATE_SUPPLIER','suppliers',$newId,null,$data);
        setFlash('success',"Supplier '{$data['name']}' added.");
...
<div class="card table-card"><div class="card-body p-4">
  <form method="POST">
    ...
    <div class="mt-4 d-flex gap-2">
      <button type="submit" class="btn btn-primary px-4"><i class="bi bi-check-lg me-1"></i>Save Supplier</button>
